    ...
    <div>
      ...
      <section className="relative flex min-h-[80vh] items-center justify-center overflow-hidden bg-neutral-950 bg-stadium bg-cover bg-center text-center">
        <div className="relative z-10 px-4">
          <img src="/logo.png" alt="AKP THUNDERz" className="mx-auto mb-6 h-40 w-40 rounded-full ring-4 ring-white object-cover" />
          <h1 className="font-display text-5xl md:text-7xl text-white drop-shadow-lg">AKP THUNDERz</h1>
          <p className="mt-3 text-lg text-white/85">Play Hard &middot; Play Smart &middot; Play Together</p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link to="/register" className="inline-flex items-center justify-center gap-2 rounded-md bg-white px-5 py-2.5 font-semibold text-gold transition hover:bg-neutral-900">Join The Club</Link>
            <Link to="/matches" className="inline-flex items-center justify-center gap-2 rounded-md border border-white px-5 py-2.5 font-semibold text-white transition hover:bg-white hover:text-gold">View Matches</Link>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 px-4 md:grid-cols-4">
          {stats.map(({ icon: Icon, label, value }) => (
            <div key={label} className="text-center">
              ...
              <p className="font-display text-3xl text-neutral-100">{value}</p>
      ...
      <section className="bg-gradient-to-r from-maroon-dark to-maroon py-14 text-center">
        <h2 className="font-display text-3xl text-white">Ready to be part of the Thunder?</h2>
        <p className="mt-2 text-white/85">Get in touch with the club or follow us on social media.</p>
        <div className="mt-6 flex justify-center gap-4">
          {[Facebook, Instagram, Youtube, Twitter, MessageCircle].map((Icon, i) => (
            <a key={i} href="#" className="rounded-full border border-white p-3 text-white transition hover:bg-white hover:text-gold">
              <Icon size={20} />
            </a>
          ))}
        </div>
        <Link to="/contact" className="mt-8 inline-flex items-center justify-center gap-2 rounded-md bg-white px-5 py-2.5 font-semibold text-gold transition hover:bg-neutral-900">Contact Us</Link>
      </section>
    </div>
  );
};
